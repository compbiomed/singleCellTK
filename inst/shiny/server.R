  ...
  output$batchCorrVar <- renderPlot({
    if (!is.null(vals$counts) &
      ...
      if (vals$batchRes[[resName]] == 'reddim'){
        ...
        # plotSCEBatchFeatureMean(inSCE = vals$counts,
        #                         useAltExp = resName, #need to decide this if we should check for altExp within function or use a getAssay function
        #                         batch = input$batchCheckVar)
        plotSCEBatchFeatureMean(inSCE = altExp(vals$counts, resName),
                                useAssay = resName, #need to decide this if we should check for altExp within function or use a getAssay function
                                batch = input$batchCheckVar)
      }
    }
  })
  ...
  output$batchCorrReddim <- renderPlot({
    if (!is.null(vals$counts) &
      ...
      if (vals$batchRes[[resName]] == 'reddim'){
        ...
        pcaName <- paste0(resName, "_PCA")
        ae <- getPCA(ae, useAssay = resName, reducedDimName = pcaName) #Need to decide this altExp issue, so we have same use throughout
        #ae <- getPCA(ae, useAssay = "altExp", reducedDimName = pcaName)
        
        #altExp should exclude reducedDim?
        #reducedDim(vals$counts, pcaName) <- reducedDim(ae, pcaName)
        #updateReddimInputs()
        
        plotSCEDimReduceColData(ae, colorBy = input$batchCheckVar,
                                shape = shapeBy, reducedDimName = pcaName,
                                title = paste0(resName, " corrected"))
      }
    }
