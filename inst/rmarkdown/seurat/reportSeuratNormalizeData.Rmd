...
```{r Seurat Normalization, message=FALSE, warning=FALSE, eval=(is.null(metadata(data)$seurat$normAssay))}
...
```{r, include=FALSE, eval=(is.null(metadata(data)$seurat$normAssay))}
normalizeParams$inSCE <- NULL
metadata(data)$seurat$sctk$report$normalizeParams <- normalizeParams
```

```{r, results="asis", eval=(!is.null(metadata(data)$seurat$normAssay))}
cat("Restored normalized data")
```
