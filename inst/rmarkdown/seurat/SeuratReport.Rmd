...
for (currentCluster in 0:(numClusters)-1) {
  cat(sprintf(template, paste0("Cluster ", as.character(unique(data.markers$cluster1))[currentCluster])))
...
if (numMarkerGenes > 0){
  ...
  cat(sprintf(template_inside, paste0("Split by sample")))
    plots <- seuratGenePlot(data, features = markerGenes[currentMarker], plotType = "feature", cols = c("grey", "blue"), groupVariable = group, splitBy = group, ncol = 2)
    plots <- ggarrange(plotlist = plots, ncol = 2, nrow = 2)
    print(plots)
  cat(space)
  cat(space)
  }
