...
```{r,results='asis', warning=FALSE, eval = !(runHVG && length(metadata(data)$seurat$obj@assays$RNA@var.features) > 0)}
hvgParams <- list(
  inSCE = data,
  hvgMethod = "vst", 
...
```{r, include=FALSE, eval=!(runHVG && length(metadata(data)$seurat$obj@assays$RNA@var.features) > 0)}
