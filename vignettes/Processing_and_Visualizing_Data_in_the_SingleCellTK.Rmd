...
library(GSVAdata)
data("c2BroadSets")
tempres <- gsva(assay(maits_subset, "logcounts"), c2BroadSets, parallel.sz=1)
fit <- limma::lmFit(tempres, stats::model.matrix(~factor(colData(maits_subset)[, "condition"])))
fit <- limma::eBayes(fit)
toptableres <- limma::topTable(fit, number = 5)
gsva_res <- tempres[rownames(toptableres), ]

if(all(colnames(maits_subset) == colnames(gsva_res))){
  colData(maits_subset) <- cbind(colData(maits_subset), data.frame(t(gsva_res)))
}
```
