...
6. A plot is procuded that displays the relationship between the mean and variabiliyt or disperson of each gene. Highly variable features will have higher levels of variability or dispersion regardless of the mean.
...
5. Select the number of PCs that should be used in the downstream clustering and 2-D embedding analyses.
...
3. Select a reduction method. Only results that were previously computed in the 'Dimensionality Reduction' tab will be available.
4. Change optional parameters if desired.
5. Press 'Run tSNE' or 'Run UMAP' button to start processing.
6. A 2-D embedding scatter plot will be generated on the right.
...
2. Select number of components to use from the previously computed PCA.
...
2. Select if you want to identify marker genes between for each cluster/group compared to all  other cluster/groups or between two specific clusters/groups.
3. Change optional parameters if desired.
4. Press "Find Markers" button to run the marker identification analysis.
5. Differentially expressed features will be displayed in the table on the right. Additional filters can be applied to further refine the list of differentially expressed genes. For example, specifying a log fold change cutoff in addition to an adjusted p-value cutoff can sometimes help identify better marker genes. 

![](ui_screenshots/scanpy/scanpy_findmarkers_1.png) <br>

6. A Heatmap plot of the top differentially expressed genes for each cluster/group can be used to visualize the patterns of expression within and across clusters/groups.

![](ui_screenshots/scanpy/scanpy_findmarkers_2.png) <br>

**7. Downstream Analysis** <br>
Once all steps of the Scanpy workflow have been completed, users can further explore the data by selecting other downstream analysis options.
...
All methods Scanpy wrapper functions in *singleCellTK* take a `SingleCellExperiment` (SCE) object as an input and will return an SCE with additional information. For this tutorial, we will use a small PBMC dataset:
...
Expression of individual marker genes can be visualized in Dot plots, Violin plots, Matrix plots and via a Heatmap:

```{r "scanpy_genePlots", warning=FALSE, message=FALSE}
plotScanpyMarkerGenes(sce, groups = '0')
plotScanpyMarkerGenesViolin(sce, groups = '0')
plotScanpyMarkerGenesHeatmap(sce, groupBy = 'Scanpy_leiden_1', nGenes = 10)
```
