...
sce <- getUMAP(sce, useReducedDim = "PCA", initialDims = 10, reducedDimName = "UMAP", seed = 12345)
```
...
Then we call `findMarkerDiffExp()` on the clustered data, with the cluster annotation just obtained, `"cluster"`.  

```{R runFM}
sce <- findMarkerDiffExp(inSCE = sce, useAssay = "logcounts", cluster = "cluster")
```
...
After successfully running `findMarkerDiffExp()`, the result marker table would be stored in `metadata(sce)`. This table can be fetched by `findMarkerTopTable()`. Argument `topN` controls how many top markers, that pass filters, to include for each clsuter.

```{R getMarkerTable}
findMarkerTopTable(sce, log2fcThreshold = 0, minClustExprPerc = 0.5, topN = 1)
```
...
plotMarkerDiffExp(sce, log2fcThreshold = 0, minClustExprPerc = 0.5, maxCtrlExprPerc = 0.5, rowLabel = TRUE)
```
