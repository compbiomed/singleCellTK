...
SCTK allows importing datasets from preprocessing tools like CellRanger, STARSolo, or simple flat files: [CellRanger](https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/what-is-cell-ranger) [@Zheng2017], [Optimus](https://data.humancellatlas.org/pipelines/optimus-workflow), [DropEst](https://github.com/hms-dbmi/dropEst) [@Petukhov2018], [BUStools](https://github.com/BUStools/bustools) [@Melsted2019], [Seqc](https://github.com/dpeerlab/seqc), [STARSolo](https://github.com/alexdobin/STAR/blob/master/docs/STARsolo.md) [@Dobin2013], [Alevin](https://salmon.readthedocs.io/en/latest/alevin.html) [@Srivastava2019] Please refer to the [advanced user guide on importing](import_data.html) on how to structure the input formats.

SCTK also supports importing a raw count matrix, with metadata in the following formats: flat files (.csv/.tsv), [SingleCellExperiment](https://rdrr.io/bioc/SingleCellExperiment/man/SingleCellExperiment.html) object saved in RDS file, [Seurat](https://satijalab.org/seurat/) object saved in RDS file (requires conversion to SingleCellExperiment), [AnnData](https://anndata.readthedocs.io/en/latest/) object saved in hdf5 file

SCTK also allows for the importing of multiple batches (samples) of dataset at the same time. 

This tutorial focuses on how to import and QC the example PBMC3k data stored in an RDS object. **For a more detailed overview, refer to the [advanced user guide on importing](import_data.html) for different data importing functions.**
...
After entering the [website](sctk.bu.edu) or starting the UI from the Shiny web application locally, the landing page will be for importing the scRNAseq data. To import the PBMC3k dataset previously mentioned, users should follow these steps:
...
Here, we briefly introduce the approach to importing the output of the widely used preprocessing tool, `CellRanger`; many users will directly get their data from 10x after running CellRanger. SCTK has a generic function `importCellRanger()` for this purpose, and, explicitly, `importCellRangerV2()` and `importCellRangerV3()` for different versions of `CellRanger`. For the detail of these functions, please click on the function names to be redirected to the reference page. **If you are following the tutorial PBMC3k data, you may skip this paragraph and move on to Quality Control.**

````{=html}
</details>
<details>
  <summary><b>Reference for importing CellRanger output</b></summary>
````
