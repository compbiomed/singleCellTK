...
After calculation, the panel for selecting the top features, **"2. Select number of variable features"** becomes enabled. A scatter plot of features, with axis of variance and mean, will also be shown on the right panel **"Plot"**.
...
For the scran SNN graph approach, the parameter `k` can always be tweaked as it affects the connectivity of the graph. When `k` is larger, stronger connection will be constructed across the cells, thus larger and fewer clusters will be identified. 
...
As we can observe from the marker heatmaps above, most of the clusters can be easily differentiated by the top marker genes of each cluster. However, "cluster 8", "cluster 6" and "cluster 3" are seemingly having less difference between each other than compared to other cell clusters, in terms of the marker genes presented in the heatmap (top 5 markers for each cluster). To further explore the differential expression between them, we could do a cluster-by-cluster DE analysis in the next section. 
...
Continued from the previous section, we will now set "cluster 8" and "cluster 6" as the two groups to perform the analysis. For a quick tutorial, we skip the full pairwise comparison. 
...
SCTK also supports GSEA method from EnrichR, which queries a given gene list and finds the significant overlap against the curated gene sets from multiple gene set libraries. For detail, please refer to [EnrichR Documentation](cnsl_enrichR.html).
