...
8. If "Compute HeatmapPlot?" is selected in step 8, you can specify how many features should be plotted in the heatmap by default. This setting can be changed later as well from the visualization panel on the right. <br> <br>
...
plotDimRed(inSCE = sce, useReduction = "redDimPCA", xAxisLabel = "PC_1", yAxisLabel = "PC2_2")

#Or use specific functions available for other plots
...
```{r "dm_console_example", message = FALSE, warning = FALSE}
  ...
  sce <- runFeatureSelection(inSCE = sce, useAssay = "counts", hvgMethod = "vst")
          ...
          tempAssay <- expData(inSCE = sce, assayName = "LogNormalizedScaledCounts")[HVGs,]
          expData(inSCE = sce, assayName = "hvg2000", tag = "hvg", altExp = TRUE) <- tempAssay
  # Run PCA
  sce <- runDimReduce(inSCE = sce, useAssay = "hvg2000", useAltExp = "hvg2000", reducedDimName = "redDimPCA", method = "seuratPCA", nComponents = 10)
  # Plot PCA
