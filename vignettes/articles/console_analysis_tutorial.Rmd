...
sce <- runDimReduce(inSCE = sce, method = "scaterPCA", 
                    useAssay = "logcounts_scaled", useAltExp = "hvg", scale = FALSE)
...
sce <- runDimReduce(inSCE = sce, method = "uwotUMAP", useReducedDim = "PCA", reducedDimName = "UMAP")
#sce <- getUMAP(sce, useReducedDim = "PCA", reducedDimName = "UMAP")
#sce <- runDimReduce(inSCE = sce, method = "seuratUMAP", useReducedDim = "PCA", reducedDimName = "seuratUMAP")
```
...
#plotSCEDimReduceColData(sce, colorBy = "cluster", reducedDimName = "seuratUMAP", dotSize = 1)
```
...
plotMarkerDiffExp(sce, topN = 5, log2fcThreshold = 0, fdrThreshold = 0.05,
                  minClustExprPerc = 0.5, maxCtrlExprPerc = 0.4,
                  minMeanExpr = 0, rowLabel = TRUE)
```

### Customized Differential Expression Analysis

As we can easily tell from the heatmap above, "cluster 7" and "cluster 5" are having a very similar expression profile across the top markers detected. In order to further explore the difference between these two clusters, or any other two reasonable groups of cells, user can use the function `runDEAnalysis()`. 
...
sce <- runDEAnalysis(inSCE = sce, method = "wilcox", class = "cluster", classGroup1 = c(5), classGroup2 = c(7),
                 groupName1 = "cluster5", groupName2 = "cluster7", analysisName = "cluster5_VS_7")
```
...
plotDEGHeatmap(sce, useResult = "cluster5_VS_7", rowLabel = TRUE)
```
