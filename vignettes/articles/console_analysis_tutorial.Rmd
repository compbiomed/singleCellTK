...
The latest version of SCTK allows users to select the score matrix from pathway analysis to perform differential expression analysis. In this process, each geneset will be treated as the feature/gene, and comparison groups can still be set in the same way as introduced in [DE analysis section](#customized-differential-expression-analysis). 
...
plotDEGHeatmap(sce, useResult = "T_cells_VS_monocytes_VAM_H_CDF", rowLabel = TRUE, rowTitle = "Geneset")
```
