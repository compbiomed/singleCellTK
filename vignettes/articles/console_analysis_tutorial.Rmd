...
```{R import, message=FALSE, warning=FALSE, results='hold', cache=TRUE}
...
```{R qc, message=FALSE, warning=FALSE, results='hold', cache=TRUE}
...
```{R plotPerCellQC, message=FALSE, warning=FALSE, cache=TRUE, fig.height=10, fig.width=8}
plotRunPerCellQCResults(sce)
```

```{R plotScDblFinder, message=FALSE, warning=FALSE, cache=TRUE, fig.height=7, fig.width=8}
plotScDblFinderResults(sce, reducedDimName = "decontX_UMAP")
```

```{R plottDecontX, message=FALSE, warning=FALSE, cache=TRUE, fig.height=7, fig.width=8}
...
```{R norm, message=FALSE, warning=FALSE, cache=TRUE, results='hold'}
sce <- runNormalization(sce, useAssay = "counts", outAssayName = "logcounts",
                        normalizationMethod = "logNormCounts")
#expData(sce, "logcounts_scaled") <- computeZScore(logcounts(sce))
#sce <- expSetDataTag(sce, "scaled", "logcounts_scaled")
...
```{R hvg, message=FALSE, warning=FALSE, cache=TRUE}
...
```{R pca, message=FALSE, warning=FALSE, results="hold", cache=TRUE}
runDimensionalityReduction(sce, useAssay = "logcounts_scaled", method = "scaterPCA")
sce <- scaterPCA(sce, useAltExp = "hvg",
                 useAssay = "logcounts_scaled", scale = FALSE)
```
...
Uniform Manifold Approximation and Projection (UMAP) is one of the most commonly used methods to visualize the relationships between the cells in a 2-D embedding. This can be done using the function `getUMAP()`, which is a wrapper of `uwot::umap()` method. The result will be saved in `reducedDim` slot.  

Following the steps above, we will now use `useReducedDim` argument to pass the PCA embedding just generated to the function. This wrapper function also allows `useAssay` or `useAltExp` + `useAssay` method, while providing an option of running PCA dimension reduction on the expression matrix prior to the UMAP computation. In this way, however, the PCA will be done by `uwot`, thus cannot be customized.  
...
```{R umap, message=FALSE, warning=FALSE, results="hold", cache=TRUE}
sce <- getUMAP(sce, useReducedDim = "PCA", reducedDimName = "UMAP")
```
...
```{R cluster, message=FALSE, warning=FALSE, results="hold", cache=TRUE}
sce <- runScranSNN(sce, useReducedDim = "PCA", clusterName = "cluster", k = 5)
```
...
```{R cluster_umap, message=FALSE, warning=FALSE, results="hold", cache=TRUE}
plotSCEDimReduceColData(sce, colorBy = "cluster", reducedDimName = "UMAP", dotSize = 1)
```

## Differential Expression and Marker Detection

SCTK has a generic function `runDEAnalysis()` for performing differential expression (DE) analysis between two selected groups of cells. By running DE analysis, the function will take the normalized read count data and perform statistical analysis to discover quantitative changes in expression levels between selected groups. See here for the detail of [running DE analysis with SCTK](differential_expression.html).  

Users can next perform marker detection process with clusters labeled, or any other types of reasonable grouping of similar biological state of cells annotated in `colData` slot. SCTK further wraps the DE analysis function to find markers for each cluster, by running DE analysis on one cluster against all other clusters and iteratively doing the same for each cluster.  

```{R findMarker, message=FALSE, warning=FALSE, results="hold", cache=TRUE}
...
```{R getMarkerTable, message=FALSE, warning=FALSE, results="hold", cache=TRUE}
topMarkers <- findMarkerTopTable(sce)
```
...
```{R markerHeatmap, message=FALSE, warning=FALSE, results="hold", cache=TRUE, fig.height= 6, fig.width=9}
plotMarkerDiffExp(sce, topN = 5, log2fcThreshold = 0, fdrThreshold = 0.05,
...
```{R singleR, message=FALSE, warning=FALSE, results="hold", cache=TRUE}
...
```{R singleRUMAP, message=FALSE, warning=FALSE, cache=TRUE, fig.width=9}
