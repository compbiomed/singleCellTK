...
sce <- seuratFindClusters(inSCE = sce, 
...
if ("percentVar" %in% names(attributes(pca))) {
    ...
    new_pca <- Seurat::CreateDimReducObject(embeddings = sce, 
                                            assay = "RNA",
                                            stdev = stdev, 
                                            key = "PC_")
} else {
...
pbmc3k <- seuratFindClusters(inSCE = sce, 
                             useAssay = assayNames(sce)[1], # Need a placeholder
