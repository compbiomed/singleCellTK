...
The function `sampleSummaryStats()` may be used to generate a table containing the mean and median of the data per sample, which is stored within the `sctk_qc` table under `metadata`. The table can then be returned using `sampleSummaryStatsTable`.

```{r sampleSummaryStats}
pbmc.combined <- sampleSummaryStats(pbmc.combined, sample = sample.vector)
sampleSummaryStatsTable(pbmc.combined, statsName = "sctk_qc")
```
