...
sce <- setTopHVG(inSCE = sce, method = "vst", featureSubsetName = "featureSubset")
...
sce <- runSeuratPCA(inSCE = sce, useAssay = "seuratNormData", reducedDimName = "seuratPCA", nPCs = 20, useFeatureSubset = "featureSubset", scale = TRUE)
...
sce <- runSeuratUMAP(inSCE = sce, useReduction = "pca", reducedDimName = "seuratUMAP", dims = 10, seed = 42)
```
```{r, warning=FALSE}
