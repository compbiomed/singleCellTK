...
There is no function special for batch correction, but this can be achieved simply by using the [dimension reduction](dimensionality_reduction.html)/[2D embedding](2d_embedding.html) calculation functions (e.g. `scaterPCA()`, `getUMAP()` and `getTSNE()`) and `plotSCEDimReduceColData()`.
...
```{r umapAfter}
sce.small <- getUMAP(inSCE = sce.small, useAssay = 'LIMMA', reducedDimName = 'UMAP_LIMMA')
