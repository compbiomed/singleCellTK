...
This guide helps users with performing batch correction when necessary. SingleCellTK provides 11 methods that are already published. A detailed referencing table can be expanded and viewed below.
...
| Method |   Function   | Result Slots |
| :----: |    :----:    |    :----:    |
| BBKNN [@Polanski2020] | `runBBKNN()` | `reducedDim` |
| ComBatSeq [@ZhangY2020]  | `runComBatSeq()` | `assay` |
| FastMNN [@Haghverdi2018]  | `runFastMNN()` | `reducedDim` |
| MNN [@Haghverdi2018] | `runMNNCorrect()` | `assay` |
| Harmony [@Korsunsky2019]  | `runHarmony()` | `reducedDim` |
| LIGER [@LiuL2020] | `runLIGER()` | `reducedDim` |
| Limma [@Ritchie2015] | `runLimmaBC()` | `assay` |
| Scanorama [@Hie2019] | `runSCANORAMA()` | `assay` |
| scMerge [@LinY2019] | `runSCMerge()` | `assay` |
| Seurat Integration [@Butler2018a] | `seuratIntegration()` | `altExp` |
| ZINBWaVE [@Risso2018] | `runZINBWaVE()` | `reducedDim` |
