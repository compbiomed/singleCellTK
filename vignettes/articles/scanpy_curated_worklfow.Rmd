...
author: "Nida Pervaiz, Irzam Sarfaraz"
output:
  html_document:
    toc: true
    toc_depth: 2
...
3. Press the 'Normalize' button to start the normalization process.

**2. Scale Data** <br>
Once normalization is complete, data needs to be scaled and centered accordingly. `Scanpy` scales data to unit variance and zero mean.

1. Open up the 'Scale Data' tab.
2. Select the `assay` to scale from the dropdown menu.
3. Press 'Scale' button to start processing.
...
5. Graph that plots each gene and its relationship based upon the selected model in (2), and identifies if a gene is highly variable or not.
...
5. Press the 'Run PCA' button to start processing.
6. Select the number of computed components that should be used in the downstream analysis e.g. in 'tSNE/UMAP' computation or with 'Clustering'. 
7. The plot area from where all computed plots can be viewed by the user.
...
4. Information displayed to the user that how many components from the selected reduction method will be used. This value can only be changed from the 'Dimensionality Reduction' tab.
...
4. Set resolution parameter value for the algorithm.
...
6. Press 'Find Clusters' button to start processing.
7. Re-computed plots with cluster labels. Only those plots are available that have previously been computed.
...
2. Select number of genes to show. Default is `10`.
3. Press "Find Markers" button to run marker identification.
4. Identified marker genes are populated in the table.
...
sce <- runScanpyFindHVG(inSCE = sce, useAssay = "scanpyScaledData", method = "seurat")
...
sce <- runScanpyPCA(inSCE = sce, useAssay = "scanpyNormData", reducedDimName = "scanpyPCA", nPCs = 20)
...
sce <- runScanpyTSNE(inSCE = sce, useReduction = "scanpyPCA", reducedDimName = "scanpyUMAP", dims = 10)
```
```{r, warning=FALSE, message=FALSE}
sce <- runScanpyUMAP(inSCE = sce, useReduction = "scanpyPCA", reducedDimName = "scanpyTSNE", dims = 10)
```
```{r, warning=FALSE}
...
sce <- runScanpyFindClusters(inSCE = sce, useAssay = "scanpyScaledData", useReduction = "scanpyPCA", dims = 10, algorithm = "louvain")
```
...
plotScanpyEmbedding(sce, reducedDimName = "scanpyPCA", color = 'Scanpy_louvain_1')
plotScanpyEmbedding(sce, reducedDimName = "scanpyTSNE", color = 'Scanpy_louvain_1')
...
sce <- runScanpyFindMarkers(inSCE = sce, colDataName = "Scanpy_louvain_1")
