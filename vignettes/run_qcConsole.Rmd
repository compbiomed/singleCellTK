...
```{r load_package, eval=TRUE, message=TRUE, warning=TRUE}
library(singleCellTK)
library(dplyr)
```


## Quality Control of Droplet Counts Data
...
```{r rundropletqc, eval=FALSE, message = FALSE}
pbmc4k.droplet <- runDropletQC(pbmc4k.droplet)
```

```{r echo=FALSE, eval=TRUE}
pbmc4k.droplet <- readRDS("/Users/rz2333/Downloads/Study/BU/Josh/git/singleCellTK/pbmc.droplet.rds")
```
...
```{r, coldata_drop2, echo=TRUE, eval=FALSE}
...
```{r table2, fig.wide = TRUE, echo=FALSE, message=TRUE, warnings=FALSE, results='asis', fig.align="center"}
tabl <- "
...
```{r, eval = TRUE, echo = FALSE, warning=TRUE, message=TRUE}
...
```{r run_barcoderanks, message=TRUE, eval=FALSE}
...
```{r load_data2, eval=FALSE, message=TRUE}
...
```{r dimensionalityreduction, eval=FALSE, warning=TRUE, message=TRUE}
# UMAP:
...
```{r load_geneset, eval=FALSE,  message = FALSE}
...
```{r echo=FALSE, eval=TRUE}
### subset 1000 cells and runcellQC done
pbmc.combined <- readRDS('/Users/rz2333/Downloads/Study/BU/Josh/git/singleCellTK/pbmc.cell.rds')
sample.vector <- colData(pbmc.combined)$sample
...
A summary of all outputs is shown below:
```{r table3, fig.wide = TRUE, echo=FALSE, message=TRUE, warnings=FALSE, results='asis', fig.align="center"}
tabl <- "
| QC output                              | Description                                                   | Methods           | Package/Tool  |
|----------------------------------------|:--------------------------------------------------------------|:------------------|:--------------|
...
```{r plot_percellqc, message=TRUE, warning=TRUE}
runpercellqc.results <- plotRunPerCellQCResults(inSCE = pbmc.combined, sample = sample.vector, combinePlot = TRUE, axisSize = 8, axisLabelSize = 9, titleSize = 9)
```

```{r fig.wide = TRUE, fig.height = 21, fig.width = 18}
...
```{r umap_scrubletplots, message=TRUE, warning=TRUE}
...
scrublet.results$pbmc6k$violinScore <- NULL
scrublet.results$pbmc3k$violinScore <- NULL

scrublet.plots<- .ggSCTKCombinePlots(scrublet.results, ncols=3)
```

```{r fig.wide = TRUE, fig.height = 18, fig.width = 15}
scrublet.plots 
```

##### DoubletCells
[DoubletCells](https://rdrr.io/github/MarioniLab/scran/man/doubletCells.html) is a doublet detection algorithm in the scran package. DoubletCells aims to detect doublets by creating a simulated doublet from existing cells and projecting it to the same PCA space as the cells.

The wrapper function plotDoubletCellsResults can be used to plot the QC outputs from the DoubletCells algorithm.

```{r umap_doubletcells_plots, message=TRUE, warning=TRUE}
doubletcells.results <- plotDoubletCellsResults(
  inSCE = pbmc.combined, sample = colData(pbmc.combined)$sample, 
  reducedDimName = "UMAP", combinePlot = TRUE
)
```
...
doubletFinderResults$pbmc6k$violin_resolution_1.5 <- NULL
doubletFinderResults$pbmc3k$violin_resolution_1.5 <- NULL

doubletFinderPlots<- .ggSCTKCombinePlots(doubletFinderResults, ncols=3)
```
...
```{r umap_hybrid_plots, eval=FALSE}
...
```{r run_scrublet, eval = FALSE, message=TRUE}
...
```{r run_doubletcells, eval = FALSE, message=TRUE}
pbmc.combined <- runDoubletCells(inSCE = pbmc.combined, sample = colData(pbmc.combined)$sample, useAssay = "counts")
```
...
```{r run_doubletfinder, eval = FALSE, message=TRUE}
...
```{r run_cxds, eval = FALSE, message=TRUE}
...
```{r run_bcds, eval = FALSE, message=TRUE}
...
```{r run_hybrid, eval = FALSE, message=TRUE}
...
```{r run_decontx, eval = FALSE, message=TRUE}
