...
data to produce an ESetSC object that can be used in the single cell toolkit.
...
```{r, eval=FALSE}
singlecellobject <- alignSingleCellData(inputfile1 = c("/path/to/sample1_1.fastq.gz","/path/to/sample2_1.fastq.gz"),
...
the samples to create an ESetSC object. The ```alignSingleCellData()``` function
can be run on an individual file. Once individual sample objects have been
created, combine them using the ```mergeSCESet()``` function.
...
## Manually Creating a Single Cell Object with createSCESet()

If you choose to align and quantify your data using alternative tools, you can
use the ```createSCESet()``` function to create a single cell object that can
...
```createSCESet()``` function help page for details.
