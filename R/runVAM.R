...
#' @description Wrapper for the Variance-adjusted Mahalanobis (VAM), which is a fast and accurate method for cell-specific gene set scoring of single cell data. This algorithm computes distance statistics and one-sided p-values for all cells in the specified single cell gene expression matrix. Gene sets should already be imported and stored in the meta data using functions such as \link{importGeneSetsFromList} or \link{importGeneSetsFromMSigDB}
...
#' 
#' @return   A \link[SingleCellExperiment]{SingleCellExperiment} object with VAM metrics stored in \code{reducedDim} as \code{VAM_NameOfTheGeneset_Distance} and \code{VAM_NameOfTheGeneset_CDF}.
#' @seealso \link{importGeneSetsFromList}, \link{importGeneSetsFromMSigDB}, \link{importGeneSetsFromGMT}, \link{importGeneSetsFromCollection} for importing gene sets.
#' @author Nida Pervaiz
#' @export
#'
#' @examples
