...
## Add the fre_expre
## make it able to change the labels being interest (only interested vs. not-interested )
## server for MAST can be modified by the differential expression tab.  add a result-table on ui/server connected to the  
## use plot_DiffEx for heatmap 

#' MAST 
#' @param SCEdata SCE dataset for running the MAST 
#' @param FCTHRESHOLD the threshold for filetering the sample 
#' @param freq_expressed frequence filtering criterion 
...
#' @param p.value  p values for selecting the hurdle reslut, default is .05
#' @export

MAST <- function(SCEdata, 
                 freq_expressed = 0.1, condition = NULL ,  interest.level = NULL, 
                 FCTHRESHOLD=log2(1.5) , p.value = .05 ){
