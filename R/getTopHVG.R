    ...
    if(method == "vst" || method == "dispersion" || method == "modelGeneVar"){
        ...
        if (method == "vst") {
            if (is.null(rowData(inSCE)$seurat_variableFeatures_vst_varianceStandardized)) {
                stop("'seurat_variableFeatures_vst_varianceStandardized' metric not found in rowData of
                    input sce object. Run Seurat feature selection with 'vst' method before using this function!")
            }
            ...
            if (is.null(rowData(inSCE)$seurat_variableFeatures_dispersion_dispersion)) {
                stop("'seurat_variableFeatures_dispersion_dispersion' metric not found in rowData of
                    input sce object. Run Seurat feature selection with 'dispersion' method before using this function!")
            }
            ...
            if (is.null(rowData(inSCE)$scran_modelGeneVar_bio)) {
                stop("'scran_modelGeneVar_bio' metric not found in rowData of
                    input sce object. Run scran feature selection with 'modelGeneVar' method before using this function!")
            }
