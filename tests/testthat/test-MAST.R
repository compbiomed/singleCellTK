...
test_that("MAST should fail if too few samples pass filter", {
  expect_error(MAST(maits_sce, condition = "condition", freqExpressed = "1",
...
test_that("MAST should fail if data has NAs", {
  expect_error(MAST(original, condition = "Primary.Type",
                    interest.level = "L4 Arf5", freqExpressed = "0.2",
                    useThresh = TRUE, useAssay = "logcounts" ),
