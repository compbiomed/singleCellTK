...
test_that("Should fail when the object supplied is not a sctk/ sctkexperiment object",{
  expect_error(visPlot(matrix(1:100),"counts","boxplot",NULL,c("Cmtm5","C1qa")),
               "Please use a singleCellTK or a SCtkExperiment object")
})

test_that("Should fail when no condition is provided",{
  expect_error(visPlot(mouseBrainSubsetSCE, "counts", "boxplot", NULL, c("Cmtm5", "C1qa")), "Please supply a condition")
})

test_that("Should fail when an incorrect plot name is provided",{
  expect_error(visPlot(mouseBrainSubsetSCE,"counts","nothingplot","level1class",c("Cmtm5","C1qa")),
               "method 'nothingplot' is not a valid method.")
})

test_that("Should fail when an incorrect assay name is provided",{
  expect_error(visPlot(mouseBrainSubsetSCE,"david","boxplot","level1class",c("Cmtm5","C1qa")),
               "assay 'david' does not exist.")
})

test_that("Should fail when an incorrect gene name is provided",{
  expect_error(visPlot(mouseBrainSubsetSCE,"counts","boxplot","level1class",c("david_gene")),
               "Gene in gene list not found in input object.")
})

test_that("Should fail when more than 1 condition is provided",{
  expect_error(visPlot(mouseBrainSubsetSCE,"counts","boxplot",c("age","diameter"),c("Cmtm5")),
               "Only 1 condition allowed")
})

test_that("Should fail when the condition is not a factor for boxplot",{
  expect_error(visPlot(mouseBrainSubsetSCE,"counts","boxplot","age",c("Cmtm5")),
...
test_that("Should fail when there are more than 16 genes for boxplot",{
  expect_error(visPlot(mouseBrainSubsetSCE,"counts","boxplot","age",c("Tspan12", "Tshz1", "Fnbp1l",
      "Adamts15", "Cldn12", "Rxfp1", "2310042E22Rik", "Sema3c", "Jam2", "Apbb1ip",
      "Frem2", "BC005764", "Deptor", "C130030K03Rik", "Klhl13", "Tnfaip8l3","Cmtm5")),
...
test_that("Should fail when the condition is a factor for scatterplot",{
  expect_error(visPlot(mouseBrainSubsetSCE,"logcounts","scatterplot","level1class",c("Cmtm5","C1qa")),
...
test_that("Should fail when the condition is NULL for scatterplot",{
  expect_error(visPlot(mouseBrainSubsetSCE,"logcounts","scatterplot", NULL, c("Cmtm5","C1qa")),
               "Please supply a condition")
})

test_that("Should fail when a condition is supplied for barplot",{
  expect_error(visPlot(mouseBrainSubsetSCE,"logcounts", "barplot", "age", c("Cmtm5","C1qa")),
...
test_that("Should fail when the input genes supplied for a heatmap has a sum of zero counts",{
  expect_error(visPlot(mouseBrainSubsetSCE,"counts", "heatmap", NULL, c("Adamts15")),
