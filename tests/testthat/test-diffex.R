...
test_that("scDiffEx and scDiffExlimma() functions should give the same result", {
  ...
  res <- scDiffEx(inSCE = nozeros, useAssay = "logcounts",
                  condition = "level1class", ntop = nrow(nozeros),
                  usesig = FALSE, diffexmethod = "limma")
  res2 <- scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
  ...
  res <- scDiffEx(inSCE = nozeros, useAssay = "logcounts",
                  ...
                  ntop = nrow(nozeros), usesig = FALSE, diffexmethod = "limma")
  res2 <- scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
                        condition = "level1class", covariates = "tissue")
  expect_equal(res, res2)
  ...
  res <- scDiffEx(inSCE = nozeros, useAssay = "logcounts",
                  condition = "level2class", ntop = nrow(nozeros),
                  analysisType = "biomarker", usesig = FALSE,
                  diffexmethod = "limma", levelofinterest = "Oligo6")
  res2 <- scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
                        condition = "level2class", analysisType = "biomarker",
  ...
  res2 <- scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
                        condition = "level2class", analysisType = "biomarker",
                        levelofinterest = "Oligo6", covariates = "tissue")
  expect_equal(res, res2)

  # nine levels, no covariates, coef
  res <- scDiffEx(inSCE = nozeros, useAssay = "logcounts",
                  condition = "level2class", ntop = nrow(nozeros),
                  analysisType = "coef", usesig = FALSE, diffexmethod = "limma",
                  levelofinterest = "Oligo6")
  res2 <- scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
                        condition = "level2class", analysisType = "coef",
                        levelofinterest = "Oligo6")
  expect_equal(res, res2)
  ...
  res <- scDiffEx(inSCE = nozeros, useAssay = "logcounts",
                  condition = "level2class", ntop = nrow(nozeros),
                  analysisType = "allcoef", usesig = FALSE,
                  diffexmethod = "limma")
  res2 <- scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
                        condition = "level2class", analysisType = "allcoef")
  expect_equal(res, res2)
  ...
  res <- scDiffEx(inSCE = nozeros, useAssay = "logcounts",
                  condition = "level2class", ntop = nrow(nozeros),
                  analysisType = "allcoef", usesig = FALSE,
                  diffexmethod = "limma", covariates = "tissue")
  res2 <- scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
                        condition = "level2class", analysisType = "allcoef",
                        covariates = "tissue")
  expect_equal(res, res2)

  expect_error(scDiffEx(inSCE = nozeros, useAssay = "logcounts",
                        condition = "level2class", ntop = nrow(nozeros),
                        analysisType = "false", usesig = FALSE,
                        diffexmethod = "limma", covariates = "tissue"),
               "Unrecognized analysis type, false")

  expect_error(scDiffExlimma(inSCE = nozeros, useAssay = "logcounts",
                             condition = "level2class", analysisType = "false",
                             covariates = "tissue"),
               "Unrecognized analysis type, false")
...
test_that("scDiffEx and scDiffExDESeq2() functions should give the same result", {
  ...
  res <- scDiffEx(inSCE = subset, useAssay = "counts",
                  condition = "level1class", ntop = nrow(subset),
                  usesig = FALSE, diffexmethod = "DESeq2")
  ...
  res <- scDiffEx(inSCE = subset, useAssay = "counts",
                  ...
                  analysisType = "biomarker", levelofinterest = "Oligo6",
                  usesig = FALSE, diffexmethod = "DESeq2")
  res2 <- scDiffExDESeq2(inSCE = subset, useAssay = "counts",
                         condition = "level2class", analysisType = "biomarker",
                         levelofinterest = "Oligo6")
  ...
  res <- scDiffEx(inSCE = subset, useAssay = "counts",
                  condition = "level2class", ntop = nrow(subset),
                  analysisType = "contrast", levelofinterest = "Oligo6",
                  controlLevel = "Oligo5", usesig = FALSE,
                  diffexmethod = "DESeq2")
  res2 <- scDiffExDESeq2(inSCE = subset, useAssay = "counts",
                         condition = "level2class", analysisType = "contrast",
...
test_that("scDiffEx and scDiffExANOVA() functions should give the same result", {
  #anova
  res <- scDiffEx(inSCE = mouseBrainSubsetSCE, useAssay = "logcounts",
                  condition = "level2class", ntop = nrow(mouseBrainSubsetSCE),
                  usesig = FALSE, diffexmethod = "ANOVA")
  ...
  res <- scDiffEx(inSCE = mouseBrainSubsetSCE, useAssay = "logcounts",
                  condition = "level2class", ntop = nrow(mouseBrainSubsetSCE),
                  usesig = FALSE, diffexmethod = "ANOVA", covariates = "tissue")
  res2 <- scDiffExANOVA(mouseBrainSubsetSCE, useAssay = "logcounts",
                        condition = "level2class", covariates = "tissue")
  expect_equal(res, res2[rownames(res), ])
})
...
test_that("NA's should thow proper error message", {
  ...
  colData(tmpdata)$testcondition <- rep(NA, ncol(tmpdata))
  expect_error(scDiffEx(tmpdata, useAssay = "logcounts", "testcondition"),
